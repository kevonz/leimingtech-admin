    						...
    						<tbody>
    							<tr>
          							<td class="bdl bdr tc">
          								...
          								<a href="javascript:void(0);" class="ncu-btn2" onclick="confirmShipHSC('${shopPointsOrder.id}','')">确认</a>
          							</td>
        						</tr>
    						</tbody>
    					</table>
...
function confirmShip(orderId){
	...
	$.ajax({
		...
		data: {'orderId':orderId,'deliverExplain':deliverExplain},
		dataType: "json",
		success:function(data) {
			if(data.success){
				layer.alert("发货成功!",{icon:1},function(){
					location.href='${base}/shopPointsOrder/shipments?orderState=40';
				});
			}else{

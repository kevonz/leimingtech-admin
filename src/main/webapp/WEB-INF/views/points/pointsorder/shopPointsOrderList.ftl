...
<div class="page">
					...
					<td colspan="4"><select name="pointOrderstate"
						class="querySelect">
							...
							<option value="20"<#if shopPointsOrder.pointOrderstate
								== '20'>selected="selected" </#if>>待付款</option>
							<option value="30"<#if shopPointsOrder.pointOrderstate
								== '30'>selected="selected" </#if>>待发货</option>
							<option value="40"<#if shopPointsOrder.pointOrderstate
								== '40'>selected="selected" </#if>>待收货</option>
							<option value="50"<#if shopPointsOrder.pointOrderstate
								== '50'>selected="selected" </#if>>已收货</option>
							<option value="60"<#if shopPointsOrder.pointOrderstate
								== '60'>selected="selected" </#if>>已完成</option>
							<option value="70"<#if shopPointsOrder.pointOrderstate
								== '70'>selected="selected" </#if>>已取消</option>
					</select>
					</td>
	...
	<form method="post" id='form_list'
		...
		<table class="table tb-type2">
			...
			<tbody>
				<#assign dateFormatTag = newTag("dateFormatTag")/> <#list
				pager.result as shopPointsOrder>
				<tr class="hover edit">
					...
					<td><#if shopPointsOrder.pointOrderstate==20> 待付款 <#elseif
						shopPointsOrder.pointOrderstate==30> 待发货 <#elseif
						shopPointsOrder.pointOrderstate==40> 待收货 <#elseif
						shopPointsOrder.pointOrderstate==50> 已收货 <#elseif
						shopPointsOrder.pointOrderstate==60> 已完成 <#elseif
						shopPointsOrder.pointOrderstate==70> 已取消 </#if></td>
					<td class="align-center">
						<a href="${base}/shopPointsOrder/detail?orderId=${shopPointsOrder.id}">查看</a>
						&nbsp;
						<#if shopPointsOrder.pointOrderstate==30> |&nbsp;
						 <a href="${base}/shopPointsOrder/shipments_deliver?orderId=${shopPointsOrder.id}">发货</a> 

            ...
            <#if datas??>
			  <#list datas as str>
			  	<tr>
			  		...
			  		<td>${str.sellerState}</td>
			  		<td>
			  			<#if str.refundState=="2">待处理</#if> 
						<#if str.refundState=="3">已完成</#if>
					</td>
			  		<td class="icon-link">
			  			<a href="javascript:operdetail(${str.refundId})"  title="查看" >查看</a>
			  		</td>
			  	</tr>
			  </#list>
			</#if>
...
<script type="text/javascript">
	...
	function operdetail(id){
		var url = "${base}/refundReturn/findByIdMoney?status=all&id="+id;

...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>权限设置</h3>
      <ul class="tab-base"><li><a href="${base}/setting/admin/index" ><span>管理员</span></a></li><li><a  class="current"><span>编辑管理员</span></a></li>
         <#--<li><a href="#?act=admin&op=gadmin" ><span>权限组</span></a></li><li><a href="#?act=admin&op=gadmin_add" ><span>添加权限组</span></a></li>-->
      </ul>    </div>
  </div>
  ...
  <form id="add_form" method="post" action="${base}/setting/admin/saveOrUpdate">
    ...
    <table class="table tb-type2 nobdb">
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform">${admin.adminName}</td>
          <td class="vatop tips">请输入登录名</td>
        </tr>
       
        <tr>
          <td colspan="2" class="required"><label class="validation" for="adminPassword">新密码:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="password" id="adminPassword" name="adminPassword" value="" class="txt"></td>
          <td class="vatop tips">请输入密码</td>
        </tr>
       
        <tr>
          <td colspan="2" class="required"><label class="validation" for="adminRpassword">确认密码:</label></td>
        </tr>
        <tr class="noborder">
          	...
          	<select name="roleid">
                 <#list rolelist as role>
                    <option value="${role.id}"
                    	<#if role.id==admin.roleid>selected="selected"</#if>>${role.name}
                    </option>
                 </#list>
            </select>
          </td>

	...
	<table class="member-list" id="acct_grid" border="0">
		...
		<tbody id="grid"> 
            ...
            <#if datas??>
			  <#list datas as str>
			  	<tr>
			  		<td><input type="checkbox" class="checkbox" value="${str.straceId}"/></td>
			  		<td>${str.straceStorename}</td>
			  		<td>${str.straceTitle}</td>
			  		<td><span class="time">${str.straceContent}</span></td>
			  		<td><span class="ip">${str.straceTime}</span></td>
			  		<td class="icon-link">
			  			<a href="#" class="icon-del" title="删除" onclick="deleteLog('${str.straceId}')">删除</a>
...
<script type="text/javascript">
	...
	function changeState(state){ 
		...
		/*开始修改操作*/
		var dataNum = bizId.split(",").length;
		var url = "${base}/tracelog/updateState";

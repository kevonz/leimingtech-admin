...
//							//id 修改内容索引标识
//							//value 修改内容
//                            $.get(modUrl,{id:s_id,value:$(this).attr('value')},function(data){
//                                if(data==false)
//                                {
//                                    alert('名称已经存在，请您换一个');
...
//		});
	...
	$('a[nc_type="inline_edit"]').click(function(){
		...
		$.get(modUrl,{id:i_id,value:i_val},function(data){
		if(data)
			{
				if(i_val == 0){
 ...
 $.fn.inline_edit= function(options) {
     ...
     function onClick() {
         ...
         .blur(function(){
             ...
             if(new_value != '') {
                 $.get(settings.act,{id:column_id,value:new_value},function(data){
                     if(data.result) {
